"""Plot Cus15 offline hyperparameter sweep against online/solver references."""
...
RUNS = [
    {
        "label": "DAPG coef=0.05 s2006",
        "run_name": "O2O_CUS15_SWEEP_DAPG_C005_ACTIONKEY_R40_U3_NE64_MB4_E500",
        "seed": 2006,
        "color": "#d62728",
        "style": "-",
        "width": 2.1,
    ...
    {
        "label": "DAPG coef=0.10 s2006",
        "run_name": "O2O_CUS15_SWEEP_DAPG_C010_ACTIONKEY_R40_U3_NE64_MB4_E500",
        "seed": 2006,
        "color": "#8c564b",
        "style": "-",
        "width": 2.1,
    ...
    {
        "label": "Gated DAPG eta=0.10 mean s2006",
        "run_name": "O2O_CUS15_SWEEP_GADAPG_C010_ETA010_MEAN_ACTIONKEY_R40_U3_NE64_MB4_E500",
        "seed": 2006,
        "color": "#ff7f0e",
        "style": "-",
        "width": 2.1,
    ...
    {
        "label": "AWBC coef=0.10 eta=0.10 s2006",
        "run_name": "O2O_CUS15_SWEEP_AWBC_C010_ETA010_ACTIONKEY_R40_U3_NE64_MB4_E500",
        "seed": 2006,
        "color": "#9467bd",
        "style": "-",
        "width": 2.1,
...
def read_train(path: Path):
    ...
    for row in read_csv(path):
        ...
        out[int(epoch)] = {
            ...
            "dapg_demo_gate_mean": _float(row.get("dapg_demo_gate_mean")),
            "dapg_memory_better_rate": _float(row.get("dapg_memory_better_rate")),
            "dapg_memory_gap_mean": _float(row.get("dapg_memory_gap_mean")),
            "awbc_weight_mean": _float(row.get("awbc_weight_mean")),
            "awbc_active_ratio": _float(row.get("awbc_active_ratio")),
            "awbc_expert_better_ratio": _float(row.get("awbc_expert_better_ratio")),
...
def main() -> None:
    ...
    parser.add_argument("--output-stem", default="cus15_offline_sweep_seed2006_canvas")
    ...
    for spec in RUNS:
        ...
        for row in eval_rows:
            row.update(train_by_epoch.get(row["epoch"], {}))
        ...
        summary.append(
            {
                ...
                "dapg_demo_gate_mean": last.get("dapg_demo_gate_mean", math.nan),
                "dapg_memory_better_rate": last.get("dapg_memory_better_rate", math.nan),
    ...
    fig, axes = plt.subplots(6, 1, figsize=(12.5, 17), sharex=True)
    metrics = [
        ("objective", "Avg objective distance (km)"),
        ("vehicle_count", "Avg vehicle count"),
        ("entropy", "Policy entropy"),
        ("bc_loss", "Demo/BC/AWBC loss"),
        ("dapg_demo_gate_mean", "DAPG demo gate mean"),
        ("awbc_weight_mean", "AWBC weight mean"),
    ]
    for ax, (metric, ylabel) in zip(axes, metrics):
        for spec, rows in loaded:
            xs, ys = [], []
            for row in rows:
                y = row.get(metric, math.nan)
                if not math.isnan(y):
                    xs.append(row["epoch"])
                    ys.append(y)
            if xs:
    ...
    axes[0].set_title("Cus15 offline hyperparameter sweep (seed 2006)")
    ...
    fields = [
        "label", "run_name", "seed", "final_epoch", "final_obj", "best_obj", "best_epoch",
        "vehicle_count", "feasible_rate", "entropy", "bc_loss", "bc_coef", "bc_accuracy",
        "dapg_demo_gate_mean", "dapg_memory_better_rate", "awbc_weight_mean", "awbc_active_ratio",
    ]
