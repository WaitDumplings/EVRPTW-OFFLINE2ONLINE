...
cmd=(
  ...
  --config "$O2O_ROOT/configs/cus50_o2o_sl_ppo_group_ref_u4_dyn_mem_dde_1000.yaml"
  --seed "$SEED"
  --device cuda
)
if (($#)); then
...
CUDA_VISIBLE_DEVICES="$CUDA_DEVICE" "${cmd[@]}" \
  > "$O2O_ROOT/results/launch_logs/cus50_slppo_group_refgap_u4_dyn_mem_dde_1000_seed_${SEED}.log" 2>&1
